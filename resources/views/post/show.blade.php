<x-layout :title="$pageTitle" xmlns="http://www.w3.org/1999/html">
  <h2>{{ $post->title }}</h2>
  <p>{{ $post->body }}</p>
  <p>{{ $post->author }}</p>
                ...
                <div>
                    ...
                    <div class="mt-2">
                        <textarea type="text" name="content" id="content" rows="4"
                               class="{{$errors->has('author') ? 'outline-red-500' : 'outline-gray-300'}} block w-full rounded-md bg-white px-3 py-1.5 text-base text-gray-900 outline-1 -outline-offset-1 placeholder:text-gray-400 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-600 sm:text-sm/6"></textarea>
                    </div>
                    @error('author')
                    <span class="text-red-500 text-sm">{{$message}}</span>
                    @enderror
